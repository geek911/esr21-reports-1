...
class ScreeningReportsViewMixin(EdcBaseViewMixin):
    ...
    def total_screened_participants(self):
        ...
        return ['Total screened',overall,gaborone,maun,serowe,f_town,phikwe]
    ...
    def screening_failure(self):
        all_screening_ids = self.total_screened.values_list('screening_identifier', flat=True)
        ...
        failed = self.total_screened.filter(~Q(screening_identifier__in=passed_screening)).count()
        gaborone = self.total_screened.filter(~Q(screening_identifier__in=passed_screening) & Q(site_id=self.get_site_id('Gaborone'))).count()
        maun = self.total_screened.filter(~Q(screening_identifier__in=passed_screening) & Q(site_id=self.get_site_id('Maun'))).count()
        serowe = self.total_screened.filter(~Q(screening_identifier__in=passed_screening) & Q(site_id=self.get_site_id('Serowe'))).count()
        francistown = self.total_screened.filter(~Q(screening_identifier__in=passed_screening) & Q(site_id=self.get_site_id('Francistown'))).count()
        phikwe = self.total_screened.filter(~Q(screening_identifier__in=passed_screening) & Q(site_id=self.get_site_id('Phikwe'))).count()
        
        data = ['Screening failure',failed,gaborone,maun,serowe,francistown,phikwe]
        return data
    ...
    def screening_failure_reasons(self):
        ...
        passed_screening = self.consent_model_cls.objects.filter(
            subject_identifier__in=vaccination).values_list('screening_identifier', flat=True)
        passed_screening = list(set(passed_screening))
        failed = self.total_screened.filter(~Q(screening_identifier__in=passed_screening))
        ...
        for fail in failed:
            if not fail.is_eligible:
                ...
                try:
                    consent = self.consent_model_cls.objects.get(screening_identifier=fail.screening_identifier)
                except self.consent_model_cls.DoesNotExist:
                    print('Mising consent ****************')
                else:
                    try:
                        second_screening = self.screening_eligibility_cls.objects.get(subject_identifier=consent.subject_identifier)
